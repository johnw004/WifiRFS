...
.TITLESTR
        EQUS    "Wifi RFS ",$00,"0.3"

.COPYR
        EQUS    $00,"(C)2023 John Wike",$00
...
\ service 13
        ...
        BNE     Not_4
        JSR     set_heap                         \ set the heap page in paged RAM
        LDA     $F4
...
.Hex_Dec                                         \ convert hex value in $100/1 to decimal value in $102 to $104
        TYA
        PHA
        LDY     #$00
        STY     $102
...
.Rw_Load_Loop
        ...
        JMP     set_heap                     \ reset ram page to heap
...
PRINT "Rw_End is at ",~(Rw_End)," versus limit of &8FF7"
\ &8FF7 = wifi version flag, 0 for old mfa, 8 for new mfa
\CRC of Etag will be stored at &8FF8, and 6 bytes are needed at the end as a buffer in case RFS fills up
    
SAVE "wrfs03",romstart,romend
